...
function Bar({ value, max, label, right }: { value: number; max: number; label: string; right: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="w-16 shrink-0 text-right text-xs tabular-nums text-ink-2">{right}</span>
    </div>
  );
}
...
export default function ReviewView() {
  ...
  if (!data) {
    return <div className="text-sm text-ink-3">复盘计算中…</div>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-md border border-line bg-surface p-5">
        <h2 className="mb-3 flex items-center gap-1.5 text-sm font-medium text-ink-2">
      ...
      <section className="grid grid-cols-3 gap-3">
        <div className="rounded-md border border-line bg-surface p-4">
          <div className="text-xs text-ink-2">总投入</div>
          <div className="mt-1 text-xl font-semibold tabular-nums text-ink">
            {formatDurationCompact(ov.totalSeconds)}
          </div>
          <div className="text-xs text-ink-3">{ov.sessionCount} 次专注</div>
        </div>
        <div className="rounded-md border border-line bg-surface p-4">
          <div className="text-xs text-ink-2">完成任务</div>
          <div className="mt-1 text-xl font-semibold tabular-nums text-ink">
            {ov.taskCompleted}
          </div>
          <div className="text-xs text-ink-3">共创建 {ov.taskCreated}</div>
        </div>
        <div className="rounded-md border border-line bg-surface p-4">
          <div className="text-xs text-ink-2">完成率</div>
          <div className="mt-1 text-xl font-semibold tabular-nums text-ink">
            {completionRate}%
          </div>
          <div className="text-xs text-ink-3">未完成 {ov.taskIncomplete}</div>
        </div>
      </section>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <section className="rounded-md border border-line bg-surface p-4">
          <h3 className="mb-3 text-sm font-medium text-ink-2">类别投入 Top</h3>
          {ov.categoryStats.length === 0 ? (
            <p className="text-xs text-ink-3">暂无数据</p>
          ) : (
        ...
        <section className="rounded-md border border-line bg-surface p-4">
          <h3 className="mb-3 text-sm font-medium text-ink-2">项目投入 Top</h3>
      ...
      {derived && derived.stalled.length > 0 && (
        <section className="rounded-md border border-warn/40 bg-warn/10/70 p-4">
