...
import TodayFocusController from "./TodayFocusController";
...
describe("TodayFocusController（紧凑状态条，不做任务选择）", () => {
  it("未运行时（IDLE）不渲染", () => {
    const { container } = render(<TodayFocusController />);
    expect(container.firstChild).toBeNull();
  });

  it("专注中显示任务名 / 剩余时间 / 状态 / 番茄进度 / 暂停与结束按钮", () => {
    usePomodoroStore.setState({
      taskId: 7,
      phase: "focus",
  ...
  it("已暂停显示继续", () => {
    usePomodoroStore.setState({
      taskId: 7,
      phase: "focus",
      snapshot: snap({ state: "PAUSED", remainingMs: 20 * 60_000 }),
    });
    render(<TodayFocusController />);
  ...
  it("走满后显示专注完成并可结束", () => {
    usePomodoroStore.setState({
      taskId: 7,
      phase: "focus",
      snapshot: snap({ state: "COMPLETED", elapsedMs: 25 * 60_000, remainingMs: 0 }),
    });
    render(<TodayFocusController />);
