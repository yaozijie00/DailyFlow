...
- **今日页提醒区重构**：昨日未完成、时间冲突/日程超载移出纵向流，进入右侧上下文列 —— 提醒卡位于**详情面板正上方**（共用一列，详情在下自然衔接；详情关闭时提醒单独占 260px 列）；出现/消失不影响 Timeline 顶部位置；窄窗口收为**页头右上铃铛入口** + 浮层，绝不整条横排或压缩时间轴
- 提醒卡支持：折叠（收成标题一行）、× 暂时收起（下次进入/数据清空自动恢复）、冲突「定位」到时间轴、≤3 条 + 查看全部、单条/全部移到今天、超载「移到明天」
- 课程表：左表格右课程对调；时间范围跟随「时间轴开始/结束」设置（默认 08:00–24:00，行高自适应，吸附跟随设置）
- 长期页：容器加宽 max-w-6xl；月历任务条加粗，每目标自动稳定色（12 色调色板，与课程表同语言）
- 设置：新增启动默认页、默认隐藏已完成、显示便签栏、周起始日（周一/周日，月历与课程表列序联动）；快捷键默认改 1~6 连号（设置 = Ctrl+6），「恢复默认值」一键迁移旧键位
- 统计/复盘：统计页指标去重与层级优化；复盘概览数字前置（含平均每次专注），新增「时段投入 Top」三栏视图
- 任务优先级（迁移 0020，tasks.priority）：高/中/低三档；表单三档选择；今日任务行彩色优先级标签 + 按优先级筛选（与分类并排不拥挤）；拖动排序按完整列表重排防乱序
- 专注循环：阶段间显式选择 —— 专注完→「去休息 X 分钟 / 结束本轮 / 完成任务」，休息完→「开始下一轮 / 结束会话」，绝不自动连开
- 成就拓展：新增 13 项 / 6 条渐进链（高优先级攻坚、早起鸟、单日专注高峰、周末战士、全分类玩家、单日任务高峰）；引擎新增 4 个条件类型（morning/weekend/daily_pomodoros/high_priority）
- 细节打磨：导航选中浅底 + 左指示条、Focus 页容器对齐、统计冗余指标清理、过期文案修正

### Technical

- 迁移 0020（幂等）；lib 新增/重构：dayWarnings（冲突/超载纯计算）、taskPriority/goalColors/courseColors、schedule 参数化（可视窗/行高/重叠检测）、monthView 周起始日
- 今日提醒重构为 ReminderRail（含单测），删除原全宽 Banner / 徽章条组件；修复「hooks 渲染数变化」崩溃（hooks 全部置于提前 return 之前）
- 测试 634/634 全绿；版本 2.3.0

## DailyFlow 2.2.0

### Features（课程表模块：交互与视觉升级）

- 添加课程双模式：**单击课程 → 点空白格放置**（顶部彩色提示条、光标变十字、悬停实时幽灵预览 + 列高亮，可连放多格；Esc / 再点课程取消）；**按住课程卡直接拖入周视图**（幽灵跟随 + 时间标签 + 目标列高亮，网格外松手取消）
- 课程块**上下边拖拽调时长**（与时间轴任务块一致）：上边改开始（结束不动）、下边改结束；15 分钟吸附、最短 30 分钟、整块不超 08:00–22:00 可视窗；实时幽灵 + 原块半透明 + 跟随时间标签；原「+30」按钮移除
- **冲突保护**：目标时段与其他课重叠 → 幽灵变红「该时段已有安排」，点击/松手均不落格
- **自动配色**：每门课按创建顺序从 12 色调色板取稳定色（零迁移）；课程库圆点 / 周视图色块（彩左条 + 淡底，与时间轴同语言）/ 本周进度图例全程同色，一眼可辨
- 左栏「我的课程」改版：色点 + 课程名 + 每周 N 节（悬停变删除 ✕）、彩色描边选中态（不再整行变黑）、行式布局与空态引导；网格空态给出两步操作提示
- 修复：拖拽不再选中文字（交互区 select-none + 拖拽 preventDefault）；落点夹取统一到可视窗内（不再可能排到屏幕外）；修正跨列换算

### Technical

- 配色纯函数 lib/courseColors.ts（courseColor，含单测）；schedule.ts 新增可视窗常量、clampGridStart / resizeSlot（上/下边调整数学，含单测）、spansOverlap / isOccupied（重叠检测，含单测）
- 组件内部状态机：armed（放置模式）/ hover（悬停目标）/ drag（library·block·resize 三类拖拽，均走 useWindowDrag：Esc / 失焦自动取消）
- 测试 614/614 全绿；版本 2.2.0
